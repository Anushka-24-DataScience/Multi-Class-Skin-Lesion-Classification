...
with gr.Blocks(title="DermaCancerScan", theme=gr.themes.Soft()) as demo:
    ...
    **Model**: EfficientNetB4 | **Dataset**: HAM10000 | **AUC**: 0.959 | **Classes**: 7
    > ⚠️ **Disclaimer**: Research tool only. Always consult a qualified dermatologist.
    """)

    with gr.Row():
        with gr.Column():
            image_input = gr.Image(type="numpy", label="Upload Dermoscopy Image")
            submit_btn  = gr.Button("🔍 Analyse Image", variant="primary", size="lg")
        with gr.Column():
            out_condition  = gr.Textbox(label="Predicted Condition")
            out_risk       = gr.Textbox(label="Risk Level")
            out_confidence = gr.Textbox(label="Confidence")
            out_rec        = gr.Textbox(label="Recommendation", lines=2)
            out_probs      = gr.Label(label="Class Probabilities", num_top_classes=7)
