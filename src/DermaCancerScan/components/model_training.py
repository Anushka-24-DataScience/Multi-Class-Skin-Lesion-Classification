...
class ModelTraining:
    ...
    def train_valid_generator(self):
        ...
        if self.config.params_is_augmentation:
            train_datagen = tf.keras.preprocessing.image.ImageDataGenerator(
                rotation_range=30,           # was 20 — more aggressive for minority classes
                width_shift_range=0.25,      # was 0.2
                height_shift_range=0.25,     # was 0.2
                shear_range=0.15,
                zoom_range=0.25,             # was 0.2
                horizontal_flip=True,
                vertical_flip=True,          # was False — skin lesions are rotation invariant
                brightness_range=[0.8, 1.2], # NEW — handles lighting/camera variation in HAM10000
                fill_mode="nearest",
                validation_split=0.20
            )
        else:
        ...
        # ── Aggressive class weights via power scaling ──
        # Standard "balanced" underweights minorities on HAM10000.
        # Power scaling (0.75 exponent) boosts DF/VASC without
        # making NV weight so small that training becomes unstable.
        counter = Counter(self.train_generator.classes)
        max_count = max(counter.values())
        self.class_weights = {
            cls: (max_count / count) ** 0.75
    ...
    def _unfreeze_top_layers(self):
        ...
        80 layers (~17%) gives the backbone enough flexibility.
        BatchNorm layers remain frozen throughout to preserve
        learned statistics.
        """
        n = self.config.params_fine_tune_layers
        for layer in self.model.layers[-n:]:
